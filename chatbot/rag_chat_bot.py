...
def main():
    ...
    while True:
        ...
        def find_foreign_chars(s: str) -> str:
            # Cyrillic + Hangul + replacement char
            bad = []
            for ch in set(s):
                o = ord(ch)
                if (0x0400 <= o <= 0x04FF) or (0xAC00 <= o <= 0xD7AF) or ch == "\ufffd":
                    bad.append(ch)
            return "".join(sorted(bad))

        for score, m in picked:
            t = m.get("text", "")
            bad = find_foreign_chars(t)
            if bad:
                print("⚠️ Found foreign/garbage chars in source:", m.get("url"), "chars:", bad)
                break
        ...
        system_msg = (
            ...
            "8) answer using the same language as the question, and only use hebrew and english. \n"
        )
